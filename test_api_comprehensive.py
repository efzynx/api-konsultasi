...
def dosen_headers(app):
    ...
    with app.app_context():
        ...
        dosen_user = User(
            username="testdosen", nama="Test Dosen", nim=None, role="dosen"
        )
        ...
        access_token = create_access_token(
            identity=str(dosen_user.id), additional_claims=additional_claims
        )
...
class TestDosenEndpoints:
    ...
    def test_create_dosen(self, client, auth_headers):
        ...
        response = client.post(
            "/api/v1/dosen/", data=json.dumps(dosen_data), headers=auth_headers
        )
...
class TestBookingEndpoints:
    ...
    def test_create_booking(self, client, auth_headers, app):
        ...
        with app.app_context():
            ...
            dosen_user = User(
                username="bookingdosen", nama="Booking Dosen", role="dosen"
            )
        ...
        response = client.post(
            "/api/v1/booking/", data=json.dumps(booking_data), headers=auth_headers
        )
...
class TestProfileEndpoints:
    ...
    def test_update_profile(self, client, auth_headers):
        ...
        response = client.put(
            "/api/v1/profile/", data=json.dumps(profile_data), headers=auth_headers
        )
