...
def get_all_dosen():
    try:
        ...
        return jsonify(
            {"success": True, "message": "Data dosen berhasil diambil", "data": result}
        )
...
def add_dosen():
    ...
    try:
        ...
        new_user_dosen = User(
            username=data["username"], nama=data["nama_dosen"], role="dosen"
        )
        ...
        return (
            jsonify(
                {"success": False, "message": f"Gagal menambahkan dosen: {str(e)}"}
            ),
            500,
        )
...
def delete_dosen(id):
    ...
    try:
        ...
        db.session.rollback()
        return (
            jsonify({"success": False, "message": f"Gagal menghapus dosen: {str(e)}"}),
            500,
        )
