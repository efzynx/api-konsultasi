...
from project.models import Booking, User, Dosen
...
def get_all_bookings():
    ...
    if user_role == 'mahasiswa':
        ...
        if not user_nim:
            return jsonify({"success": False, "message": "NIM tidak ditemukan untuk mahasiswa"}), 400
        ...
        if not dosen_profile:
            return jsonify({"success": False, "message": "Profil dosen tidak ditemukan"}), 404
...
def update_booking(id):
    ...
    if user_role == 'mahasiswa' and booking.nim != claims.get('nim'):
        return jsonify({"success": False, "message": "Akses ditolak: Anda bukan pemilik booking ini"}), 403
    
    dosen_profile = Dosen.query.filter_by(user_id=user_id).first()
    if user_role == 'dosen' and (not dosen_profile or booking.dosen_id != dosen_profile.id):
        return jsonify({"success": False, "message": "Akses ditolak: Anda bukan dosen yang bersangkutan"}), 403
    ...
    if user_role == 'dosen':
        if 'status' in data and data['status'] in ['approved', 'rejected', 'pending']:
            ...
            return jsonify({"success": False, "message": "Dosen hanya dapat mengubah status"}), 400
    elif user_role == 'mahasiswa':
        booking.tanggal = datetime.strptime(data.get('tanggal', str(booking.tanggal)), '%Y-%m-%d').date()
        booking.jam = datetime.strptime(data.get('jam', str(booking.jam)), '%H:%M:%S').time()
