...
class TestAPIFlow:
    ...
    def test_complete_user_registration_and_login_flow(self, client, app):
        ...
        login_response = client.post(
            "/login", json={"username": "flowtest", "password": "flowpassword"}
        )
    ...
    def test_dosen_and_booking_flow(self, client, app):
        ...
        with app.app_context():
            dosen_user = User(
                username="dosen_flow", nama="Dosen Flow Test", role="dosen"
            )
        ...
        login_response = client.post(
            "/login", json={"username": "student_flow", "password": "studentpass"}
        )
    ...
    def test_authentication_error_handling(self, client):
        ...
        for endpoint in protected_endpoints:
            ...
            assert response.status_code in [
                401,
                422,
            ], f"Endpoint {endpoint} with invalid token returned {response.status_code}"
