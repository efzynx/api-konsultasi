...
class APITester:
    ...
    def test_user_registration(self):
        ...
        try:
            response = self.session.post(f"{self.api_base}/auth/register", json=test_user)
            success = response.status_code in [200, 201]
            data = response.json() if response.content else None
            self.log_test("User Registration", success, f"Status: {response.status_code}, Data: {data}")
    ...
    def test_dosen_registration_and_login(self):
        ...
        try:
            # Register dosen user
            response = self.session.post(f"{self.api_base}/auth/register", json=test_dosen_user)
            if response.status_code in [200, 201]:
                ...
                login_response = self.session.post(f"{self.api_base}/auth/login", json=login_data)
                if login_response.status_code == 200:
                    login_data_response = login_response.json()
                    if 'data' in login_data_response and 'access_token' in login_data_response['data']:
                        dosen_token = login_data_response['data']['access_token']
                        self.log_test("Dosen Registration & Login", True, "Dosen token obtained")
                        return True, dosen_token
                        
            self.log_test("Dosen Registration & Login", False, "Failed to create dosen user")
            return False, None
        except Exception as e:
    ...
    def test_user_login(self, user_data):
        ...
        try:
            ...
            self.log_test("User Login", success, f"Status: {response.status_code}, Token received: {bool(self.auth_token)}")
            return success
        except Exception as e:
    ...
    def test_dosen_endpoints(self):
        ...
        success, response_data = self.test_protected_endpoint("/dosen", "POST", dosen_data)
    ...
    def test_booking_endpoints(self):
        ...
        success, response_data = self.test_protected_endpoint("/booking", "POST", booking_data)
        ...
        if success and response_data and 'data' in response_data:
            ...
            if booking_id:
                ...
                self.test_protected_endpoint(f"/booking/{booking_id}", "PUT", update_data)
    ...
    def test_error_scenarios(self):
        ...
        try:
            response = self.session.post(f"{self.api_base}/auth/register", json=invalid_user)
            success = response.status_code in [400, 422]
            self.log_test("Invalid Registration Data", success, f"Status: {response.status_code}")
        except Exception as e:
    ...
    def test_swagger_documentation(self):
        ...
        try:
            ...
            if success:
                ...
                self.log_test("OpenAPI Specification", success, f"Status: {response.status_code}, Endpoints: {endpoints_count}")
            else:
                self.log_test("OpenAPI Specification", success, f"Status: {response.status_code}")
        except Exception as e:
